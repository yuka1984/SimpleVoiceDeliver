...
class NArray
{
    ...
    static Uint8ArrayCopy(sourceArray: Uint8Array, sourceIndex: number, destinationArray: Uint8Array, destinationIndex: number, length: number): void
    {
        for (var index = 0; index < length; index++) {
            destinationArray[index + destinationIndex] = sourceArray[index + sourceIndex]
        }
    ...
    static Clear<T> (array :T[], index: number, length: number)
    {
    ...
    static Uint8ArrayClear(array: Uint8Array, index: number, length: number) {
        for (var i = 0; i < length; i++) {
            array[i] = 0;
        }  
    }
